...
This module provides core training functionality for neural networks:
- TrainConfig: Configuration dataclass for training parameters
- TrainResult: Container for training results and history
- train_model: Train a single model
- train_ensemble: Train multiple models with optional resampling
- make_loaders: Create train/val DataLoaders from arrays
"""
...
def train_model(
    ...
    """
    Train a PyTorch model with cross-entropy loss.
    ...
    Args:
        ...
        y: Labels array (N,)
        cfg: Training configuration
        callback: Optional callback(epoch, loss) called each epoch
...
def train_ensemble(
    ...
    Args:
        ...
        resample_fn: Optional callable(idx) -> (X, y) for resampling
    ...
    for idx in range(num_models):
        ...
        def model_cb(epoch: int, loss: float) -> None:
            if callback:
